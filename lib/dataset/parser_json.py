...
def label_is_text_box(make_data):
    ...
    if '文本' in label or '横式' in label or '解题过程' in label:
        ...
        print('error label ', make_data['label'])
        return None, None
...
def parser_jsons(jsons_dir):
    ...
    for img_data in tqdm(images_data):
        ...
        if len(mark_datas) == 1:
            continue
        img_name = jsons_dir_name + '_' + str(index_name) + '.jpg'
...
if __name__ == "__main__":
    # RAW_JSONS_DIR_list = [
    #                       #'/share/zzh/raw_train_jsons/0909_12156',
    #                       '/share/zzh/raw_train_jsons/q1_data',
    ...
    #                      ]
    import glob
    json_dir = '/hostpersistent/zzh/dataset/text/raw_data/q2_data/*'
    RAW_JSONS_DIR_list = ['/hostpersistent/zzh/dataset/test/raw_data/v2']#['/hostpersistent/zzh/dataset/text/raw_data/q2_data/0']#glob.glob(os.path.join(json_dir))
    # RAW_JSONS_DIR_list = ['/hostpersistent/zzh/dataset/text/raw_data/q1_data']

    for RAW_JSONS_DIR in RAW_JSONS_DIR_list:
        name = RAW_JSONS_DIR.split('/')[-1]
        print('parser ', name)

        SAVE_DIR = os.path.join('/hostpersistent/zzh/dataset/test/test_data/v2_org', name)
        IMG_DIR = os.path.join(SAVE_DIR, 'imgs')
        JSON_DIR = os.path.join(SAVE_DIR, 'jsons')
        SHOW_DIR = os.path.join(SAVE_DIR, 'show')
        make_dir(IMG_DIR)
