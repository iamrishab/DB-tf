...
def get_boxes_info_from_txt(label_path):
    ...
    return np.array(boxes_temp_list, dtype=np.float32), label
...
if __name__ == '__main__':
    ...
    # train_data = json.loads(open('/share/zzh/train_json/train.json', encoding='utf-8').read(), encoding='bytes')
    img_dir = '/hostpersistent/zzh/dataset/text/parser_data/v3_data/v3_0916/imgs'
    label_dir = '/hostpersistent/zzh/dataset/text/parser_data/v3_data/v3_0916/jsons'

    save_img_dir = '/hostpersistent/zzh/dataset/text/aug_data/long-10k/imgs'
    save_label_dir = '/hostpersistent/zzh/dataset/text/aug_data/long-10k/jsons'
    save_show_dir = '/hostpersistent/zzh/dataset/text/aug_data/long-10k/show'
    ...
    for i in tqdm.tqdm(range(10000)):
        ...
        base_name = img_name.split('.')[0] + '_long_' + str(i)
        j += 1
        ...
        cut_dataset_list = clip_img(img, boxes, cls)

        # 长文本
        img, bboxes, cls = create_long_text(cut_dataset_list)
        if img is None:
            print(img_name)
            continue
