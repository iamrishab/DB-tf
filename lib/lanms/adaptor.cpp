...
namespace lanms_adaptor {
	...
	std::vector<std::vector<double>> merge_quadrangle_n9(
			py::array_t<double, py::array::c_style | py::array::forcecast> quad_n9,float lanms_threshold,
			float nms_threshold, float img_width, float long_merge_threshold) {
		...
		return polys2floats(lanms::merge_quadrangle_n9(ptr, n, lanms_threshold, nms_threshold, img_width, long_merge_threshold));
	}
